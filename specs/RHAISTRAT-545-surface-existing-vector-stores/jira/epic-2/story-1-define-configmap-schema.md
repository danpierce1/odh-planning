...
The ConfigMap uses a stores.yaml key containing a YAML list of vector store definitions. The schema should mirror llamastack config fields as closely as possible, keeping Gen AI Studio-specific fields (e.g. display name) grouped under a custom_gen_ai block — the same pattern Nick used when defining the external models schema (gen-ai-aa-external-models ConfigMap): https://gist.github.com/NickGagan/564d1209acf0452ccd0a3a15e91bed51

> **Note:** owner and domain tags have been removed from scope for this release and will not be included in the custom_gen_ai block. These may be added in a future iteration.
...
- [ ] The ConfigMap schema fields mirror llamastack config fields as closely as possible, with Gen AI Studio-specific fields (e.g. display name) grouped under a custom_gen_ai block — consistent with the pattern used for the gen-ai-aa-external-models ConfigMap. Note: owner and domain are excluded from scope for this release.
- [ ] The ConfigMap schema is defined and documented, covering required fields (type, connection endpoint, collection/index name) and optional fields (description, name, Secret reference for credentials). Note: owner and domain tags are not included in this release — may be added in a future iteration.
